...
test.describe('list page', () => {
	...
	test('shows a button to sign in with Apple when not signed in', async ({ page, browserName }) => {
		...
		const button = page.getByRole('button', { name: 'Sign in with Apple' });
		await expect(button).toBeVisible();
		await button.click();
		...
		const signInWithAppleDotComButton = page.getByRole('button', {
			name: 'Sign in with Apple.com'
		});
		await expect(signInWithAppleDotComButton).toBeVisible();
		await signInWithAppleDotComButton.click();
	...
	test('links an account created via sign in with email when signing in with Apple using the same email', async ({
		...
		const signInWithGoogleButton = page.getByRole('button', { name: 'Sign in with Apple' });
		await expect(signInWithGoogleButton).toBeVisible();
		await signInWithGoogleButton.click();
